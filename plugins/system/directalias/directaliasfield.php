...
class JFormFieldDirectaliasfield extends JFormFieldText
{
	...
	protected function getInput()
	{
		...
		if (version_compare(JVERSION, '4.0', 'l'))
		{
			...
			$html[] = '<input type="button" onclick="toggleDirectAlias(this)" value="' . Text::_($direct ? 'PLG_SYSTEM_DIRECT_ALIAS_DIRECT' : 'PLG_SYSTEM_DIRECT_ALIAS_RELATIVE') . '" class="btn hasPopover" title="<b>' . Text::_('PLG_SYSTEM_DIRECT_ALIAS_DIRECT_TIP_TITLE') . '" data-content="' . Text::_('PLG_SYSTEM_DIRECT_ALIAS_DIRECT_TIP_DESC') . '" style="cursor:pointer" data-placement="bottom" />';
			$html[] = '<input type="button" onclick="toggleAbsentAlias(this)" value="' . Text::_($absent ? 'PLG_SYSTEM_DIRECT_ALIAS_ABSENT' : 'PLG_SYSTEM_DIRECT_ALIAS_PRESENT') . '" class="btn hasPopover" title="<b>' . Text::_('PLG_SYSTEM_DIRECT_ALIAS_ABSENT_TIP_TITLE') . '" data-content="' . Text::_('PLG_SYSTEM_DIRECT_ALIAS_ABSENT_TIP_DESC') . '" style="cursor:pointer" data-placement="bottom" />';
			$html[] = '</div>';
		}
		else
		{
